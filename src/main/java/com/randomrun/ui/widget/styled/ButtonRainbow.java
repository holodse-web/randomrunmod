...
public class ButtonRainbow extends ButtonWidget {
    ...
    private AnimationDirection animationDirection = AnimationDirection.VERTICAL_UP;
    ...
    public ButtonRainbow(int x, int y, int width, int height, Text message, PressAction onPress, int index, float delay) {
        super(x, y, width, height, message, onPress, DEFAULT_NARRATION_SUPPLIER);
        this.delay = delay;
        this.creationTime = System.currentTimeMillis();
    }
    
    public ButtonRainbow setAnimationDirection(AnimationDirection direction) {
        this.animationDirection = direction;
        return this;
    }
    
    public ButtonRainbow setColors(int baseColor, int hoverColor) {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (animationDirection == AnimationDirection.VERTICAL_UP) {
            // Slide up from below
            int slideOffset = (int) ((1f - easedProgress) * 30);
            animatedY = getY() + slideOffset;
        } else {
            // Slide from left (horizontal)
            int slideOffset = (int) ((1f - easedProgress) * 50);
            animatedX = getX() - slideOffset;
        }
        ...
        // Draw background
        context.fill(animatedX, animatedY, animatedX + width, animatedY + height, finalBgColor);
        
        // Draw animated border (1.5px) - ALWAYS VISIBLE and RAINBOW
        renderAnimatedBorder(context, animatedX, animatedY, animatedX + width, animatedY + height, alpha);
        
        // Draw text
        MinecraftClient client = MinecraftClient.getInstance();
        int textAlpha = (int) (alpha * 255);
        int textColor = (textAlpha << 24) | 0xFFFFFF; // White text as requested
        ...
        if (hoverProgress > 0.01f) {
            ...
            context.fill(animatedX, animatedY, animatedX + width, animatedY + height, whiteOverlay);
        }
    }
    ...
    private int getRainbowColor(float t, float alpha) {
        int r = (int) (Math.sin(t * Math.PI * 2) * 127 + 128);
        int g = (int) (Math.sin((t + 0.33f) * Math.PI * 2) * 127 + 128);
        int b = (int) (Math.sin((t + 0.66f) * Math.PI * 2) * 127 + 128);
