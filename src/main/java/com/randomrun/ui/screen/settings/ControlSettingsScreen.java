...
public class ControlSettingsScreen extends AbstractRandomRunScreen {
    ...
    protected void init() {
        ...
        int buttonHeight = 20;
        int startY = 60;
        // int spacing = 30; // Unused
        
        // Start Method Button
        // Кнопка выбора метода старта
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (config.getStartMethod() == ModConfig.StartMethod.KEYBIND) {
            ...
            float alpha = Math.min(1.0f, (elapsed - 200) / 300.0f);
            if (alpha < 0) alpha = 0;
            
            if (alpha > 0) {
                int alphaInt = (int)(alpha * 255);
                int textColor = (color & 0x00FFFFFF) | (alphaInt << 24);
                context.drawCenteredTextWithShadow(textRenderer, Text.literal(text), centerX, startY + 25, textColor);
            }
        }
    }
}
