...
public class InterfaceSettingsScreen extends AbstractRandomRunScreen {
    ...
    protected void init() {
        ...
        /*
        String blurText = config.isBlurEnabled() ? "§a" + Text.translatable("randomrun.on").getString() : "§c" + Text.translatable("randomrun.off").getString();
        addDrawableChild(new ButtonDefault(
            centerX - buttonWidth / 2, startY + 25,
            buttonWidth, buttonHeight,
            Text.translatable("randomrun.settings.blur", blurText),
            button -> {
                config.setBlurEnabled(!config.isBlurEnabled());
                RandomRunMod.getInstance().saveConfig();
                refreshScreen();
            },
