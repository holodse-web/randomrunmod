...
public class HudRenderer implements HudRenderCallback {
    ...
    public void onHudRender(DrawContext context, float tickDelta) {
        ...
        // Only render if run is active
        if (status == RunDataManager.RunStatus.INACTIVE) return;
        ...
        if (status == RunDataManager.RunStatus.FROZEN) {
            ...
            String goText = "/go";
            context.getMatrices().push();
            context.getMatrices().scale(0.8f, 0.8f, 1.0f); // Чуть меньше (было 0.85)
            int goWidth = (int)(client.textRenderer.getWidth(goText) * 0.8f);
            int goX = (int)((x + (HUD_SIZE - goWidth) / 2) / 0.8f);
            int goY = (int)((y + HUD_SIZE - 10) / 0.8f); // Ниже (было -12)
            ...
            if (timeLimit > 0) {
                // Time Challenge mode - show countdown
            ...
            context.getMatrices().push();
            context.getMatrices().scale(0.7f, 0.7f, 1.0f); // Меньше (было 0.75)
            int scaledTimeWidth = (int)(client.textRenderer.getWidth(timeStr) * 0.7f);
            int timeX = (int)((x + (HUD_SIZE - scaledTimeWidth) / 2) / 0.7f);
            int timeY = (int)((y + HUD_SIZE - 10) / 0.7f); // Ниже (было -12)
            context.drawTextWithShadow(client.textRenderer, "§e" + timeStr, timeX, timeY, 0xFFFFFF);
            context.getMatrices().pop();
        }
    }
