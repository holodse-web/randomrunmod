...
public class TimeSelectionScreen extends AbstractRandomRunScreen {
    ...
    protected void init() {
        ...
        if (slotMachineActive && elapsed < SLOT_MACHINE_DURATION) {
            ...
            addDrawableChild(new StyledButton2(
                ...
                200, 20,
                Text.translatable("randomrun.button.back"),
                button -> MinecraftClient.getInstance().setScreen(new SpeedrunScreen(new MainModScreen(null)))
            ));
        }
    }
    ...
    private void startTimeSlotMachine() {
        ...
        if (useDifficulty) {
            selectedTime = difficulty.getRandomTime();
        } else {
    ...
    public void startSpeedrunWithTime(int timeSeconds) {
        if (parent instanceof ItemRevealScreen itemRevealScreen) {
            itemRevealScreen.startSpeedrun(timeSeconds * 1000L);
        }
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Render item name below 3D item
        String itemName = targetItem.getName().getString();
        context.drawCenteredTextWithShadow(textRenderer, itemName, centerX, centerY - 80, 0xFFFFFF);
