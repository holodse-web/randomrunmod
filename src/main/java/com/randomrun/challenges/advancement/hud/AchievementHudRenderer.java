...
public class AchievementHudRenderer implements HudRenderCallback {
    ...
    public void onHudRender(DrawContext context, RenderTickCounter tickCounter) {
        ...
        // Рендерить только если забег активен И цель - ДОСТИЖЕНИЕ
    ...
    private void renderToastHud(DrawContext context, MinecraftClient client, RunDataManager runManager, RunDataManager.RunStatus status) {
        ...
        // Вычисление позиции с анимацией слайда - ВЕРХНИЙ ЛЕВЫЙ УГОЛ
        int targetX = 5;
        int startX = -width;
        ...
        // Получение данных о достижении из кэшированной информации в RunDataManager
        ...
        if (runManager.getTargetAdvancementId() != null) {
            ...
            // Использовать кэшированную информацию отображения, сохраненную при запуске испытания
            ...
            if (cachedName != null && cachedIcon != null) {
                ...
                // Резервный вариант: попытка получить из менеджера достижений (если доступен)
        ...
        context.getMatrices().translate(x + 18, y + 16, 0); // Смещено немного вправо (было 16)
        context.getMatrices().scale(2.0f, 2.0f, 1f); // Увеличено с 1.5f
        context.getMatrices().translate(-8, -8, 0);
        context.drawItem(icon, 0, 0);
        context.getMatrices().pop();
        
        // Draw achievement name in yellow
        // Отрисовка названия достижения желтым цветом
        ...
        context.drawText(client.textRenderer, displayName, x + 40, y + 7, 0xFFFFFF00, false); // Текст смещен вправо (было 35)
        
        // Draw timer below in white
        // Отрисовка таймера ниже белым цветом
        String timeStr;
